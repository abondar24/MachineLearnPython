...
def model(data):
    # input 28 x 28
    conv = tf.nn.conv2d(data, weights1, [1, 1, 1, 1], padding='VALID')
    bias = tf.nn.relu(conv + biases1)

    # input 24 x 24
    pool = tf.nn.avg_pool(bias, [1, 2, 2, 1], [1, 2, 2, 1], padding='VALID')

    # input 12 x 12
    conv = tf.nn.conv2d(pool, weights2, [1, 1, 1, 1], padding='VALID')
...
with graph.as_default():
    ...
    weights3 = tf.Variable(tf.truncated_normal([size3 * size3 * depth, num_hidden], stddev=0.1))
