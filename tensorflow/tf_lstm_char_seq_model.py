...
class Model():
    def __init__(self, args, infer=False):
        self.args = args
        if infer:
            # when we sample, the batch and sequence len are =1
            args.batch_size = 1
            args.seq_length = 1

        # define internal cell struct
        cell_fn = rnn.BasicLSTMCell
        cell = cell_fn(args.rnn_size, state_is_tuple=True)
        self.cell = cell = rnn.MultiRNNCell([cell] * args.num_layers, state_is_tuple=True)

        # build inputs and outputs placeholders, start with zero internal vals
        ...
        with tf.variable_scope('rnnlm'):
            ...
            with tf.device("/cpu:0"):
                ...
                inputs = [tf.unstack(input_, [1]) for input_ in inputs]
        ...
        outputs, last_state = seq2seq.dynamic_rnn_decoder(inputs, self.initial_state,
                                                          cell, loop if infer else None, scope='rnnlm')
