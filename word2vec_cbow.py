...
# generate a training batch for countinuous bag of words model
def generate_batch(batch_sz, bag_win):
    global data_index
    # [bag_window target bag_window]
    span = 2 * bag_win + 1
    btch = np.ndarray(shape=(batch_sz, span - 1), dtype=np.int32)
    lbels = np.ndarray(shape=(batch_sz, 1), dtype=np.int32)
    ...
    for i in range(batch_sz):
        ...
        lbels[i, 0] = buffer_list.pop(bag_win)
        btch[i] = buffer_list
        # iter to the next buffer
        buffer.append(data[data_index])
        data_index = (data_index + 1) % len(data)
    return btch, lbels
...
for bag_window in [1, 2]:
    data_index = 0
    batch, labels = generate_batch(batch_sz=4, bag_win=bag_window)
...
with graph.as_default(), tf.device('/cpu:0'):
    ...
    loss = tf.reduce_mean(tf.nn.sampled_softmax_loss(softmax_weights, softmax_biases, tf.reduce_sum(embeds, 1),
